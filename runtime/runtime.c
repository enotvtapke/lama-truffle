...
extern int   STD(tagHash) (char*);

void *STDV(sysargs);

// Gets a raw tag
extern int STD(kindOf) (void *p) {
...
// Compare sexprs tags
extern int STD(compareTags) (void *p, void *q) {
  data *pd, *qd;
...
void* STD(i__Infix_58) (void *p, void *q) {
  void *res;
  ...
  push_extra_root(&p);
  push_extra_root(&q);
  res = Bsexp (BOX(3), p, q, UNBOX (STD(tagHash) ("cons"))); //BOX(848787));
  pop_extra_root(&q);
  pop_extra_root(&p);
...
// Functional synonym for built-in operator "!!";
int STD(i__Infix_3333) (void *p, void *q) {
...
// Functional synonym for built-in operator "&&";
int STD(i__Infix_3838) (void *p, void *q) {
  ASSERT_UNBOXED("captured &&:1", p);
  ASSERT_UNBOXED("captured &&:2", q);
...
// Functional synonym for built-in operator "==";
int STD(i__Infix_6161) (void *p, void *q) {
  return BOX(p == q);
}

// Functional synonym for built-in operator "!=";
int STD(i__Infix_3361) (void *p, void *q) {
  ASSERT_UNBOXED("captured !=:1", p);
  ASSERT_UNBOXED("captured !=:2", q);
...
// Functional synonym for built-in operator "<=";
int STD(i__Infix_6061) (void *p, void *q) {
  ASSERT_UNBOXED("captured <=:1", p);
  ASSERT_UNBOXED("captured <=:2", q);
...
// Functional synonym for built-in operator "<";
int STD(i__Infix_60) (void *p, void *q) {
  ASSERT_UNBOXED("captured <:1", p);
  ASSERT_UNBOXED("captured <:2", q);
...
// Functional synonym for built-in operator ">=";
int STD(i__Infix_6261) (void *p, void *q) {
  ASSERT_UNBOXED("captured >=:1", p);
  ASSERT_UNBOXED("captured >=:2", q);
...
// Functional synonym for built-in operator ">";
int STD(i__Infix_62) (void *p, void *q) {
  ASSERT_UNBOXED("captured >:1", p);
  ASSERT_UNBOXED("captured >:2", q);
...
// Functional synonym for built-in operator "+";
int STD(i__Infix_43) (void *p, void *q) {
  ASSERT_UNBOXED("captured +:1", p);
  ASSERT_UNBOXED("captured +:2", q);
...
// Functional synonym for built-in operator "-";
int STD(i__Infix_45) (void *p, void *q) {
  if (UNBOXED(p)) {
    ASSERT_UNBOXED("captured -:2", q);
    return BOX(UNBOX(p) - UNBOX(q));
  }
...
// Functional synonym for built-in operator "*";
int STD(i__Infix_42) (void *p, void *q) {
  ASSERT_UNBOXED("captured *:1", p);
  ASSERT_UNBOXED("captured *:2", q);
...
// Functional synonym for built-in operator "/";
int STD(i__Infix_47) (void *p, void *q) {
  ASSERT_UNBOXED("captured /:1", p);
  ASSERT_UNBOXED("captured /:2", q);
...
// Functional synonym for built-in operator "%";
int STD(i__Infix_37) (void *p, void *q) {
  ASSERT_UNBOXED("captured %:1", p);
  ASSERT_UNBOXED("captured %:2", q);
...
extern char* de_hash (unsigned);

extern int STD(tagHash) (char *s) {
  char *p;
  int  h = 0, limit = 0;
  ...
  //  if (strcmp (s, de_hash ((unsigned) h)) != 0) {
  //  failure ("%s <-> %s\n", s, de_hash(h));
  // }
...
static void printValue (void *p) {
  ...
  if (UNBOXED(p)) printStringBuf ("%d", UNBOX(p));
    ...
    case SEXP_TAG: {
#ifndef DEBUG_PRINT
      char * tag = de_hash ((unsigned) TO_SEXP(p)->tag);
#else
      char * tag = de_hash ((unsigned) GET_SEXP_TAG(TO_SEXP(p)->tag));
#endif      
...
extern int STD(uppercase) (void *v) {
  ASSERT_UNBOXED("Luppercase:1", v);
  return BOX(toupper ((int) UNBOX(v)));
}

extern int STD(lowercase) (void *v) {
  ASSERT_UNBOXED("Llowercase:1", v);
  return BOX(tolower ((int) UNBOX(v)));
}

extern int STD(matchSubString) (char *subj, char *patt, int pos) {
...
extern void* STD(substring) (void *subj, int p, int l) {
  data *d = TO_DATA(subj);
...
extern struct re_pattern_buffer* STD(regexp) (char *regexp) {
...
extern int STD(regexpMatch) (struct re_pattern_buffer *b, char *s, int pos) {
  int res;
...
extern void* STD(stringInt) (char *b) {
  int n;
  sscanf (b, "%d", &n);
  return (void*) BOX(n);
}

extern int STD(hash) (void *p) {
  return BOX(0x3fffff & inner_hash (0, 0, p));
}

extern int STD(flatCompare) (void *p, void *q) {
  if (UNBOXED(p)) {
    if (UNBOXED(q)) {
      return BOX (UNBOX(p) - UNBOX(q));
    }
...
extern int STD(compare) (void *p, void *q) {
  ...
  if (UNBOXED(p)) {
    ...
    if (is_valid_heap_pointer (p)) {
      if (is_valid_heap_pointer (q)) {
        ...
        for (; i<la; i++) {
          int c = STD(compare) (((void**) a->contents)[i], ((void**) b->contents)[i]);
...
extern void* STD(makeArray) (int length) {
  data *r;
  int n;
...
extern void* STD(makeString) (int length) {
  int   n = UNBOX(length);
  data *r;
...
extern void* STD(stringcat) (void *p) {
  void *s;
...
//extern void* Bsta (int i, void *v, void *x) {
extern void* Bsta (void *x, void *i, void *v) {
  if (UNBOXED(i)) {
...
extern void STD(failure) (char *s, ...) {
  va_list args;
...
extern void* /*Lstrcat*/ STD(i__Infix_4343) (void *a, void *b) {
...
extern void* STD(sprintf) (char * fmt, ...) {
  va_list args;
  void *s;
...
extern void* STD(getEnv) (char *var) {
  char *e = getenv (var);
  void *s;
...
extern int STD(system) (char *cmd) {
  return BOX (system (cmd));
}

extern void STD(fprintf) (FILE *f, char *s, ...) {
...
extern FILE* STD(fopen) (char *f, char *m) {
  FILE* h;
...
extern void STD(fclose) (FILE *f) {
  ASSERT_BOXED("fclose", f);

  fclose (f);
}

extern void* STD(readLine) () {
  char *buf;
...
extern void* STD(fread) (char *fname) {
  FILE *f;
  ...
  if (f) {
    if (fseek (f, 0l, SEEK_END) >= 0) {
      long size = ftell (f);
      void *s   = STD(makeString) (BOX(size));
...
extern void STD(fwrite) (char *fname, char *contents) {
  FILE *f;
...
extern void* STD(fexists) (char *fname) {
  FILE *f;
...
extern void* STD(fst) (void *v) {
  return Belem (v, BOX(0));  
}

extern void* STD(snd) (void *v) {
  return Belem (v, BOX(1));  
}

extern void* STD(hd) (void *v) {
  return Belem (v, BOX(0));  
}

extern void* STD(tl) (void *v) {
  return Belem (v, BOX(1));  
}
...
extern int STD(random) (unsigned n) {
...
void set_args (int argc, char *argv[]) {
  ...
  STDV(sysargs) = p;
  push_extra_root ((void**)&STDV(sysargs));
#ifdef DEBUG_PRINT
  print_indent ();
  printf ("set_args: end\n", n, &p, p); fflush(stdout);
  indent--;
...
extern void STD(enableGC) () {
  enable_GC = 1;
}

extern void STD(disableGC) () {
  enable_GC = 0;
}
...
static void printFromSpace (void) {
  ...
  while (cur < from_space.current) {
    ...
    case SEXP_TAG:
      s = (sexp *) d;
      d = (data *) &(s->contents);
      char * tag = de_hash ((unsigned) GET_SEXP_TAG(s->tag));
...
extern size_t __start_custom_init, __stop_custom_init;
...
int main (int argc, char* argv[]) {
  init_item *init_current = (init_item*) &__start_custom_init,
            *init_stop    = (init_item*) &__stop_custom_init;
  ...
  do
   init_current++->entry_point ();
  while (init_current != init_stop);
